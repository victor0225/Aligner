...
function buildIdeaCruiseTaskInput({ completedPages = [], currentTasks = [], meetingRecord = '' }) {
  return [
    ...
    'Use Korean.',
    'Convert roadmap topics and meeting materials into task candidates only when the work needs time outside the meeting.',
    '시간을 써야만 해결되는 일만 task로 만든다.',
    'Compare roadmap topics with current in-process tasks by goal, done criteria, needed information, assignee, status, and expected output.',
    ...
    'Each task must be grounded in the supplied roadmap topic text or meeting materials. Do not reuse canned hardware/audio examples unless the source actually mentions them.',
...
export function createOpenAiIdeaCruiseTaskGenerator({ apiKey, model = 'gpt-5.4', fetchImpl = globalThis.fetch } = {}) {
  if (!apiKey) {
    throw new Error('OpenAI IDEA CRUISE task generator requires an API key');
  }

  if (!fetchImpl) {
    throw new Error('OpenAI IDEA CRUISE task generator requires fetch');
  }
  ...
  async function generate({ completedPages = [], currentTasks = [], meetingRecord = '' }) {
    const response = await fetchImpl(OPENAI_RESPONSES_URL, {
      ...
      body: JSON.stringify({
        ...
        input: [
          {
            role: 'user',
            content: buildIdeaCruiseTaskInput({ completedPages, currentTasks, meetingRecord })
          }
        ],
        text: {
          format: {
            type: 'json_schema',
            name: 'idea_cruise_task_candidates',
            strict: true,
            schema: IDEA_CRUISE_TASK_SCHEMA
          }
        }
      })
    });
    ...
    if (!response.ok) {
      ...
      throw new Error(`OpenAI IDEA CRUISE task generation failed: ${response.status} ${body}`);
    }

    const outputText = extractOutputText(await response.json());
    if (!outputText) {
      throw new Error('OpenAI IDEA CRUISE task generation failed: empty response');
    }
